 ...
 * <p>
 *     This registry is responsible for storing all known modded network packets, and for handling the negotiation of
 *     modded network packets between the client and the server.
 * </p>
 * <p>
 *     Additionally, this registry is responsible for handling all modded network packets that are not natively known
 *     once they arrive at the receiving end.
 * </p>
 * <p>
 *     To prevent payloads from being send to a client that has no idea what to do with them, the registry provides
 *     endpoints for the vanilla code base to check if a packet can be send to a client.
 * </p>
 */
@ApiStatus.Internal
public class NetworkRegistry {
     ...
     * <p>
     *     This fires an event on the mod bus to allow mods to register their custom packets.
     *     And then stores the registered packets in the registry.
     * </p>
     * <p>
    ...
    /**
     * Invoked by a {@link ServerCommonPacketListener} when a modded packet is received on a modded connection that
     * is not natively known to the vanilla code base.
     * <p>
     *     The method will first validate that a proper modded connection is setup and that a {@link NetworkPayloadSetup} is present on the connection.
     *     If that is not the case a warning is logged, and the client is disconnected with a generic error message.
     * </p>
     * <p>
     *     If the connection is setup properly, the method will check if the packet is known to the connection, and if it is not, the client is disconnected.
     *     Then checks are executed against the stored known packet handlers to see if the packet is known to the server. Technically, this list is considered fixed,
     *     based on the fact that the registration event is only fired once during bootstrap, so in practice this is just a safe-guard against people messing
     *     with the registration map.
     *     Once that completes the registered handler is invoked.
     * </p>
    ...
    /**
     * Invoked by a {@link ClientCommonPacketListener} when a modded packet is received on a modded connection that
     * is not natively known to the vanilla code base.
     * <p>
     *     The method will first validate that a proper modded connection is setup and that a {@link NetworkPayloadSetup} is present on the connection.
     *     If that is not the case a warning is logged, and the client is disconnected with a generic error message.
     ...
     * <p>
     *     If the connection is setup properly, the method will check if the packet is known to the connection, and if it is not, the client is disconnected.
     *     Then checks are executed against the stored known packet handlers to see if the packet is known to the client. Technically, this list is considered fixed,
     *     based on the fact that the registration event is only fired once during bootstrap, so in practice this is just a safe-guard against people messing
     *     with the registration map.
     *     Once that completes the registered handler is invoked.
     * </p>
     ...
     * <p>
     *     This method determines what the versions of each of the channels are, and checks if the client and server
     *     have a compatible set of network channels.
     * </p>
     * <p>
     *     If the negotiation fails, a custom packet is send to the client to inform it of the failure, and which will
     *     allow the client to disconnect gracefully with an indicative error screen.
     * </p>
     * <p>
     *     This method should only be invoked for modded connections.
     *     Use {@link #onVanillaConnectionDetectedAtServer(ServerConfigurationPacketListener)} to indicate that during the configuration
     *     phase of the network handshake between a client and the server, a vanilla connection was detected.
     * </p>
     *
     ...
     * </p>
     * <p>
     *     If this method is invoked before the negotiation during the configuration phase has completed, and as such no {@link NetworkPayloadSetup} is present
     *     then it will only allow {@link ModdedNetworkQueryPayload} packets to be sent.
     * </p>
     *
     ...
     * The negotiation happens solely on the server side, and the result is later transmitted to the client.
     *
    ...
    /**
     * Invoked by the client when no {@link ModdedNetworkQueryPayload} has been received, but instead a
     * {@link BrandPayload} has been received as the first packet during negotiation in the configuration phase.
     * <p>
     *     If this happens then the client will do a negotiation of its own internal channel configuration, to check if mods
     *     are installed which <span class="strong">need</span> a modded connection to the server.
    ...
    private record EventLoopSynchronizedWorkHandler(ReentrantBlockableEventLoop<?> eventLoop) implements ISynchronizedWorkHandler {
